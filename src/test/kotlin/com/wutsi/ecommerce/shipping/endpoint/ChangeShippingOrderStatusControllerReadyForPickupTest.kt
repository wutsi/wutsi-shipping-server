...
class ChangeShippingOrderStatusControllerReadyForPickupTest : AbstractSecuredController() {
    @LocalServerPort
    val port: Int = 0
    ...
    fun created() {
        ...
        val response = rest.postForEntity(
            url,
            ChangeStatusRequest(
                status = ShippingOrderStatus.READY_FOR_PICKUP.name,
                "no_inventory",
        ...
        verify(eventStream).publish(
            EventURN.SHIPPING_READY_FOR_PICKUP.urn,
        ...
        assertEquals(ShippingOrderStatus.READY_FOR_PICKUP, statuses[0].status)
