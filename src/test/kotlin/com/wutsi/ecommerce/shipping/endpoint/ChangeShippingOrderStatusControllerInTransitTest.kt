...
class ChangeShippingOrderStatusControllerInTransitTest : AbstractSecuredController() {
    @LocalServerPort
    val port: Int = 0
    ...
    fun created() {
        ...
        rest.postForEntity(
            url,
            ChangeStatusRequest(
                status = ShippingOrderStatus.IN_TRANSIT.name,
                "no_inventory",
        ...
        assertEquals(ShippingOrderStatus.IN_TRANSIT, order.status)

        verify(eventStream).publish(
            EventURN.SHIPPING_IN_TRANSIT.urn,
        ...
        assertEquals(ShippingOrderStatus.IN_TRANSIT, statuses[0].status)
    ...
    fun readyForPickup() {
        ...
        assertEquals(ShippingOrderStatus.READY_FOR_PICKUP, order.status)
