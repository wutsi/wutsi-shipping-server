...
class ChangeShippingOrderStatusControllerDeliveredTest : AbstractSecuredController() {
    @LocalServerPort
    val port: Int = 0
    ...
    fun created() {
        ...
        val ex = assertThrows<HttpClientErrorException> {
            rest.postForEntity(
                url,
                ChangeStatusRequest(
                    status = ShippingOrderStatus.DELIVERED.name,
                ),
                Any::class.java
            )
        }
        ...
        assertEquals(ShippingOrderStatus.CREATED, order.status)
    ...
    fun inTransit() {
        ...
        verify(eventStream).publish(
            EventURN.SHIPPING_DELIVERED.urn,
        ...
        assertEquals(ShippingOrderStatus.DELIVERED, statuses[0].status)
